...
def menu_view(request):
    ...
    if form_pizza.is_valid():
        ...
        new_pizza.save()
        messages.add_message(request, messages.INFO, "Пицца добавленая!")
...
def add_to_cart(request, item_type, item_id, item_bigger, add_cheese):
    ...
    if item_type == "sub":
        ...
        new_sub.save()
        messages.add_message(request, messages.INFO, "Ингредиенты добавлены!")
        ...
        new_platter.save()
        messages.add_message(request, messages.INFO, "Блюда добавлены!")
        ...
        new_pasta.save()
        messages.add_message(request, messages.INFO, "Паста добавлена!")
        ...
        new_salad.save()
        messages.add_message(request, messages.INFO, "Салат добавлен!")

    else:
        return HttpResponseNotFound('<h1>Продукт не найден</h1>')
...
def make_order(request):
    ...
    messages.add_message(request, messages.INFO, f"Номер заказа {new_proper_order.id} отправлен! Если у вас есть вопросы, свяжитесь с официантом")
...
@login_required
def payment(request):
    context = {"payment": reversed(ProperOrder.objects.filter(order_client=request.user))}
    return render(request, 'orders/payment.html', context)
    pass

@login_required()
def payment_confirmation(request):
    messages.add_message(request, messages.INFO, 'Способ оплаты выбран. Спасибо!')
    return redirect(menu_view)
    pass